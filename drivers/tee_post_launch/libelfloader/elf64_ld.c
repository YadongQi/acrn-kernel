...
bool elf64_get_segment_info(const elf64_ehdr_t *ehdr,
				uint16_t segment_no, elf_segment_info_t *p_info)
{
	...
	if (segment_no < ehdr->e_phnum) {
		...
		p_info->address = (char *)phdr->p_paddr;
...
static bool elf64_update_rela_section(uint16_t e_type, uint64_t relocation_offset, elf64_dyn_t *dyn_section, uint64_t dyn_section_sz)
{
	...
	if (!dyn_section){
		pr_err("failed to read dynamic section from file\n");
		return false;
	}
	...
	if (NULL == rela
		...
		if (e_type == ET_DYN) {
			pr_err("for DYN type relocation section is optional\n");
			return true;
		}else {
			pr_err("for EXEC type missed mandatory dynamic information\n");
			return false;
		}
	}
	...
	for (i = 0; i < rela_sz / rela_entsz; ++i) {
		...
		case 0:        /* do nothing */
			break;
		default:
			pr_err("Unsupported Relocation %llx\n", rela[i].r_info & 0xFF);
			return false;
		}
	}
 ...
 */
bool elf64_load_executable(module_file_info_t *file_info, uint64_t *p_entry)
{
	elf64_ehdr_t *ehdr;
	uint8_t *phdrtab;
	uint64_t phsize;
	...
	if (0 != (low_addr & PAGE_4K_MASK)) {
		pr_err("failed because kernel low address "
			"not page aligned, low_addr = %llx\n", low_addr);
		return false;
	}
	...
	if (file_info->runtime_total_size < file_info->runtime_image_size ||
		...
		pr_err("memory is smaller than required or it is zero\n");
		return false;
	}
	...
	for (i = 0; i < (uint16_t)ehdr->e_phnum; ++i) {
		...
		if (!image_copy((void *)(uint64_t)(addr + relocation_offset),
			...
			pr_err("failed to read segment from file\n");
			return false;
		}
	...
	if (offset_0_addr != (uint64_t)~0) {
		if (offset_0_addr != low_addr) {
			pr_err("elf header is relocated to wrong place\n");
			return false;
		}
	...
	pr_info("e_e=%llx, re_off=%llx\n", ehdr->e_entry, relocation_offset);
