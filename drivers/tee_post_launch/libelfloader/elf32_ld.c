...
bool elf32_get_segment_info(const elf32_ehdr_t *ehdr,
				uint16_t segment_no, elf_segment_info_t *p_info)
{
	...
	if (segment_no < ehdr->e_phnum) {
		...
		p_info->address = (char *)(ulong)phdr->p_paddr;
...
static bool elf32_update_rela_section(uint32_t relocation_offset, elf32_dyn_t *dyn_section, uint64_t dyn_section_sz)
{
	...
	if ((NULL != rela)
		...
		for (i = 0; i < rela_sz / rela_entsz; ++i) {
			...
			case 0: /* do nothing */
				break;
			default:
				pr_err("Unsupported Relocation. rlea.r_info = %#x\n", rela[i].r_info & 0xFF);
				return false;
			}
		}
	...
	if ((NULL != rel) && (NULL != symtab)
		...
		for (i = 0; i < rel_sz / rel_entsz; ++i) {
			...
			default:
				pr_err("Unsupported Relocation, rel.r_info = %#x\n", rel[i].r_info & 0xFF);
				return false;
			}
		}
 ...
 */
bool elf32_load_executable(module_file_info_t *file_info, uint64_t *p_entry)
{
	...
	if (0 != (low_addr & PAGE_4K_MASK)) {
		pr_err("Failed because kernel low address is not page aligned, low_addr = %x", low_addr);
		return false;
	}
	...
	if (file_info->runtime_total_size < file_info->runtime_image_size ||
		...
		pr_err("dest memory is smaller than required or it is zero\n");
		return false;
	}
